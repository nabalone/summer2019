...
SPECIFIED = ['C:/Users/Faith/Desktop/noey2019summer/ps1hosts\psc100014.4.fits']
...
#TODO: ircs or fk5
        ...
        def recursiveExtraction(attemptedThresh):
            ...
            global kronrad
            global blacklist
            objects = sep.extract(swappedData, attemptedThresh,  err=bkg.globalrms, #var=noise_data2 ,
...
#TODO remove repitition of calculations between filter nums    
            ...
            try:
                z = float(zdict[idNumString])
            except KeyError: # no redshift for this event:
                z = None
#TODO which z to use? 100014, 300220, and 380108 have not in zdict   
#                100014, 150381, 360140 have not in sdss
            dA = cosmo.angular_diameter_distance(z)
            # Mpc^2
            area = objects['npix'][bestCandidate] * (degPerPix*(np.pi/180.)*dA)**2
            dL = cosmo.luminosity_distance(z) # in megaparsecs
            absMag = magnitude - 5*np.log10(dL) + 25
            separationMpc = separation * (np.pi/180.)*dA
                
        
            finalProperties = [kronrad[bestCandidate], 
                               separationMpc[bestCandidate],
                               area[bestCandidate],
                               separationMpc[bestCandidate] / area[bestCandidate],
