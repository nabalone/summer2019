...
def load(test_ind, mask=False):   
    ...
    for i in range(5):
        ...
        if i == 1:
            ...
            global TEST_BC
            TEST_BC = raw[i]
        raw = np.concatenate([raw[:i], raw[i+1:]])
...
#        np.savez(filname+extrastring, X_full, Xsep_full, y_full)
    ...
    return(X_full, y_full)#(X_test, y_test, Xsep_test, X_train, y_train, Xsep_train, X_val, y_val, Xsep_val)   
...
#            all_data =  np.load("aug_all_fold_%s.npz" % fold_num) #load()
        ...
        X_full_train, y_full_train = load(BC_NUM)
        X_full_test = [TEST_BC]
        y_full_test = [1]
