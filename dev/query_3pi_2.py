...
def main():
    ...
    for r in data.iterrows():
        row = r[1]
        index, name, typ, ra, dec, z, _phot = row
        ra = ra.split(',')[0]            
        dec = dec.split(',')[0]            
        z = z.split(',')[0]            
        print(1)
        ...
        dec = coords.dec.deg
        for filt in ['g', 'r', 'i', 'z']:
            idNumString = pad(index)
            filename = "3pi_hosts/3pi%s.%s.fits" % (idNumString, filter_to_num[filt])
            try:
                run(ra, dec, filt, filename)
            except Exception as e:
                print("ERROR: %s, %s, %s" % (filename, ra, dec))
                print(e)
