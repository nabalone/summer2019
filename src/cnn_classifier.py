...
parser.add_argument('--alt', action='store_true', dest='use_alt',
    help='use alternate shuffle (seed_offset 10)')
...
def load(answers=False, ia_only=False, three=False, seed_offset=0, crop=False, mask=False):
...
#        return
    ...
    if ia_only:
        ...
        aug_to = [290, 40, 180, 45, 25]
    elif three:
        #aug_to = [286, 183, 231, 55, 103]
        aug_to = [290, 185, 235, 55, 105]
    else:
        aug_to = [290]*5
...
#TODO augment up to appropriate numbers
        ...
        testi, testp = augment(raw[:div], raw_sep[:div], int(0.2*NUM))
        vali, valp = augment(raw[div:2*div], raw_sep[div:2*div], int(0.2*NUM))
    ...
    if ia_only:
        ...
        np.savez("aug_load_output_mask"+extrastring, X_test, y_test, Xsep_test, X_train, y_train, Xsep_train, X_val, y_val, Xsep_val)   
    elif mask:
        
    else:
        np.savez("aug_load_output3"+extrastring, X_test, y_test, Xsep_test, X_train, y_train, Xsep_train, X_val, y_val, Xsep_val)   
...
#TODO restore
    if ia_only:
        ...
        all_data = np.load("aug_load_output10.npz")
        print("loaded")
    elif args.mask:
